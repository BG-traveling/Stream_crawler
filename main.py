...
st.write('크롤링된 내용은 하단의 표에서 확인 가능하며, csv파일로 다운로드받아 엑셀로 열어보실 수 있습니다.')
...
with st.expander('상세 검색 조건', expanded=True):
    ...
    with col1:
        search_text = st.text_input('검색어', placeholder='예 : 파이썬, 인공지능')

        except_text = st.text_input('제외할 검색어', placeholder='예 : 야간 근무, 잦은 출장')
    ...
    with col2:
        if site_select == '사람인':
            ...
            selected_location = st.multiselect('지역을 선택하세요.',
                                #딕셔너리인 loc_options의 '키'만 모아서 리스트화
